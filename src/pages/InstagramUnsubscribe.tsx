...
export default function InstagramUnsubscribe() {
  const [params] = useSearchParams();
  const [state, setState] = useState<"loading" | "done" | "invalid">("loading");
  const [id, setId] = useState("");

  useEffect(() => {
    const v = (params.get("id") || params.get("ig") || "").trim();
    if (!v) { setState("invalid"); return; }
    setId(v);
    supabase.from("ig_suppression").insert({ ig_id: v, reason: "unsubscribe", source: "public" }).then(() => setState("done"));
  }, [params]);
      ...
      <div className="max-w-md w-full text-center border border-border p-8 rounded">
        ...
        {state === "done" && (
          <>
            ...
            <p className="text-sm text-muted-foreground break-all">{id} will no longer receive Instagram DMs from POSHPLEX.</p>
          </>
        )}
      </div>
    </div>
