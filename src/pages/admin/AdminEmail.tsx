...
type Settings = {
  ...
  api_key: string; from_email: string; from_name: string; reply_to: string;
  success_keyword: string; enabled: boolean; notes: string;
};
type Template = { id: string; event_key: string; name: string; subject: string; html: string; enabled: boolean; is_system: boolean };
...
export default function AdminEmail() {
  ...
  const [bulkHtml, setBulkHtml] = useState("<p>Hi {name},</p><p>Your message here.</p>");
  ...
  const load = async () => {
    ...
    const [{ data: s }, { data: t }, { data: c }, { data: m }, { data: sup }, { data: mt }, { data: dv }] = await Promise.all([
      supabase.from("email_provider_settings").select("*").maybeSingle(),
      ...
      supabase.from("email_messages").select("*").order("created_at", { ascending: false }).limit(50),
      supabase.from("email_suppression").select("*").order("created_at", { ascending: false }).limit(100),
  ...
  const saveTemplate = async (tpl: Template) => {
    const { error } = await supabase.from("email_templates").update({
      subject: tpl.subject, html: tpl.html, enabled: tpl.enabled, name: tpl.name,
    }).eq("id", tpl.id);
    if (error) toast.error(error.message); else toast.success("Template saved");
  };

  const sendBulk = async () => {
    if (!bulkSubject.trim() || !bulkHtml.trim()) { toast.error("Subject and body required"); return; }
    setSending(true);
    const audience_filter: any = { type: audType };
    if (audType === "membership" || audType === "division") audience_filter.ids = audIds;
    if (audType === "manual") audience_filter.emails = audEmails.split(/[\s,]+/).filter(Boolean);
    const { data, error } = await supabase.functions.invoke("email-send", {
      body: { action: "bulk", name: bulkName, subject: bulkSubject, html: bulkHtml, audience_filter, site_url: window.location.origin },
    });
    setSending(false);
    ...
    setBulkName(""); setBulkSubject(""); setAudEmails(""); setAudIds([]);
    load();
  };

  const removeSuppression = async (id: string) => {
    const { error } = await supabase.from("email_suppression").delete().eq("id", id);
  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto">
      <h1 className="text-2xl font-bold uppercase tracking-tight mb-1">Email Marketing</h1>
      <p className="text-sm text-muted-foreground mb-4">Bulk campaigns, transactional templates, provider settings, history and suppression. Lightweight & isolated.</p>
      ...
      <Tabs defaultValue="bulk">
        ...
        <TabsContent value="bulk" className="space-y-3 mt-4">
          <div className="border p-4 rounded-md space-y-3">
            ...
            <Input placeholder="Subject line" value={bulkSubject} onChange={e => setBulkSubject(e.target.value)} />
            <Textarea rows={8} placeholder="HTML body — use {name} for personalisation" value={bulkHtml} onChange={e => setBulkHtml(e.target.value)} className="font-mono text-xs" />
            <div>
              <Label className="text-xs mb-1 block">Live preview</Label>
              <iframe title="preview" className="w-full h-56 border bg-white rounded" srcDoc={bulkHtml} />
            </div>
            <div className="grid md:grid-cols-2 gap-3">
              <div>
                ...
                <Select value={audType} onValueChange={(v: any) => { setAudType(v); setAudIds([]); }}>
                  ...
                  <SelectContent>
                    <SelectItem value="all">All active customers (with email)</SelectItem>
              ...
              {audType === "manual" && (
                <Textarea rows={4} placeholder="Emails (comma or newline separated)" value={audEmails} onChange={e => setAudEmails(e.target.value)} />
              )}
            </div>
        ...
        <TabsContent value="templates" className="space-y-3 mt-4">
          ...
          {templates.map((tpl, i) => (
            <div key={tpl.id} className="border p-4 rounded-md space-y-2">
              ...
              <Input placeholder="Subject" value={tpl.subject} onChange={e => setTemplates(arr => arr.map((t, j) => j === i ? { ...t, subject: e.target.value } : t))} />
              <Textarea rows={4} className="font-mono text-xs" value={tpl.html} onChange={e => setTemplates(arr => arr.map((t, j) => j === i ? { ...t, html: e.target.value } : t))} />
              <p className="text-xs text-muted-foreground">Placeholders: {"{name}, {email}, {order_number}, {total}, {tracking}, {product}"}</p>
        ...
        <TabsContent value="settings" className="space-y-3 mt-4">
          <div className="border p-4 rounded-md space-y-3">
            ...
            <Input placeholder="Provider name (Resend, Mailgun, SendGrid, Brevo…)" value={settings.provider_name || ""} onChange={e => setSettings(s => ({ ...s!, provider_name: e.target.value }))} />
            <Input placeholder="Endpoint URL (supports {api_key} {from_email} {to} {subject} {html})" value={settings.endpoint_url || ""} onChange={e => setSettings(s => ({ ...s!, endpoint_url: e.target.value }))} />
            ...
            <Input placeholder="From email" value={settings.from_email || ""} onChange={e => setSettings(s => ({ ...s!, from_email: e.target.value }))} />
            <Input placeholder="From name" value={settings.from_name || ""} onChange={e => setSettings(s => ({ ...s!, from_name: e.target.value }))} />
            <Input placeholder="Reply-to (optional)" value={settings.reply_to || ""} onChange={e => setSettings(s => ({ ...s!, reply_to: e.target.value }))} />
            <Input placeholder="Success keyword (optional — e.g. 'id')" value={settings.success_keyword || ""} onChange={e => setSettings(s => ({ ...s!, success_keyword: e.target.value }))} />
            <div>
        ...
        <TabsContent value="campaigns" className="mt-4">
          <div className="border rounded-md overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-muted"><tr><th className="text-left p-2">Name</th><th>Subject</th><th>Recipients</th><th>Sent</th><th>Failed</th><th>Status</th><th>When</th></tr></thead>
              <tbody>
                {campaigns.map(c => (
                ...
                {!campaigns.length && <tr><td colSpan={7} className="p-4 text-center text-muted-foreground">No campaigns yet.</td></tr>}
              </tbody>
            </table>
          </div>
        </TabsContent>
        ...
        <TabsContent value="history" className="mt-4">
          <div className="border rounded-md overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-muted"><tr><th className="text-left p-2">To</th><th className="text-left">Subject</th><th>Status</th><th>When</th></tr></thead>
              <tbody>
                {messages.map(m => (
                  <tr key={m.id} className="border-t">
                    <td className="p-2 font-mono text-xs">{m.to_email}</td>
                ...
                {!messages.length && <tr><td colSpan={4} className="p-4 text-center text-muted-foreground">No emails sent yet.</td></tr>}
              </tbody>
            </table>
          </div>
        </TabsContent>
        ...
        <TabsContent value="suppression" className="mt-4">
          <div className="border rounded-md overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-muted"><tr><th className="text-left p-2">Email</th><th>Reason</th><th>When</th><th></th></tr></thead>
              <tbody>
                {suppression.map(s => (
                ...
                {!suppression.length && <tr><td colSpan={4} className="p-4 text-center text-muted-foreground">No suppressed addresses.</td></tr>}
              </tbody>
            </table>
          </div>
        </TabsContent>
