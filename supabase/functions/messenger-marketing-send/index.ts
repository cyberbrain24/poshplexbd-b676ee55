// Messenger Marketing send endpoint - admin only.
...
async function resolveAudience(sb: any, filter: any): Promise<{ psid: string; name?: string }[]> {
  const type = filter?.type || "all";
  if (type === "manual") {
    const ids: string[] = Array.isArray(filter.ids_text) ? filter.ids_text : [];
    return ids.map((s) => String(s).trim()).filter(Boolean).map((psid) => ({ psid }));
  }
  const { data } = await sb.from("msgr_subscribers").select("psid, name").eq("opted_in", true).limit(10000);
  return (data || []).map((s: any) => ({ psid: s.psid, name: s.name }));
}
...
async function sendOne(settings: any, to: string, body: string, mediaUrl: string) {
  const vars: Record<string, string> = {
    ...
    messaging_type: settings.messaging_type || "MESSAGE_TAG",
    message_tag: settings.message_tag || "POST_PURCHASE_UPDATE",
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: settings } = await sb.from("msgr_provider_settings").select("*").maybeSingle();
    if (!settings || !settings.enabled) return json({ error: "Messenger provider not configured/enabled" }, 400);

    if (action === "single") {
      if (!to || !body) return json({ error: "to + body required" }, 400);
      const { data: sup } = await sb.from("msgr_suppression").select("psid").eq("psid", to).maybeSingle();
      if (sup) {
        await sb.from("msgr_messages").insert({ to_psid: to, body, status: "suppressed", template_key: template_key || null });
        return json({ sent: 0, failed: 0, suppressed: 1 });
      }
      const r = await sendOne(settings, to, body, media_url || "");
      await sb.from("msgr_messages").insert({
        to_psid: to, body, template_key: template_key || null,
    ...
    if (action === "bulk") {
      ...
      const psids = recipients.map((r) => r.psid);
      const { data: suppressed } = await sb.from("msgr_suppression").select("psid").in("psid", psids);
      const sset = new Set((suppressed || []).map((s: any) => s.psid));
      const valid = recipients.filter((r) => !sset.has(r.psid));

      const { data: campaign } = await sb.from("msgr_campaigns").insert({
      ...
      for (const r of valid) {
        try {
          ...
          const res = await sendOne(settings, r.psid, personalised, media_url || "");
          if (res.ok) sent++; else failed++;
          await sb.from("msgr_messages").insert({
            campaign_id: campaign?.id, to_psid: r.psid, body: personalised,
          ...
          failed++;
          await sb.from("msgr_messages").insert({
            campaign_id: campaign?.id, to_psid: r.psid, body, status: "failed",
            error: String(e?.message || e).slice(0, 500),
          });
        }
      }

      await sb.from("msgr_campaigns").update({ sent_count: sent, failed_count: failed, status: "sent" }).eq("id", campaign!.id);
